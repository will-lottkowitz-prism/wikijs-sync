...
// A cheap "where does this page id live now, and when was it last touched"
// probe — used before an update to catch a server-side edit (updatedAt) or a
// local move/copy (path) without fetching the whole page.
export async function getPageStub(
  url: string,
  token: string,
  id: number
): Promise<{ updatedAt: string; path: string } | undefined> {
  const data = await graphqlRequest<{
    pages: { single: { updatedAt: string; path: string } | null };
  }>(
    url,
    token,
    'query($id: Int!) { pages { single(id: $id) { updatedAt path } } }',
    { id }
  );
  return data.pages.single ?? undefined;
}
