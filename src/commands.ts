  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      for (const filePath of eligible) {
        ...
        try {
          const remotePath = meta.path || pagePathForFile(ctx, filePath);
          const remote = remoteByPath.get(remotePath);
          if (remote) matchedRemotePaths.add(remotePath);

          // No remote counterpart yet — first upload (create).
          if (!remote) {
            await pushLocal();
            stats.uploaded++;
            continue;
      ...
      for (const [remotePath, page] of remoteByPath) {
        if (matchedRemotePaths.has(remotePath)) continue;
...
async function syncUpload(
  ...
  ctx: SyncContext,
  filePath: string,
  opts: { silent: boolean; skipConflictPrompt?: boolean }
) {
...
async function syncUploadLocked(
  ...
  const locationPath = pagePathForFile(ctx, filePath);
  if (!meta.path) {
    meta.path = locationPath;
  } else if (meta.path !== locationPath) {
    if (!meta.id) {
      out.appendLine(
        `note: ${filePath} front matter path "${meta.path}" != file location "${locationPath}"; using file location`
      );
      meta.path = locationPath;
    } else {
      const choice = await vscode.window.showWarningMessage(
        `Wiki.js Sync: "${path.basename(filePath)}" has front matter path "${meta.path}" (id ${meta.id}) but lives at "${locationPath}". ` +
          `If you copied this file from another page, uploading as-is will overwrite page id ${meta.id} instead of creating a new page.`,
        { modal: true },
        'This is a rename (keep id, move page)',
        'This is a new page (create, drop id)'
      );
      if (choice === 'This is a rename (keep id, move page)') {
        meta.path = locationPath;
      } else if (choice === 'This is a new page (create, drop id)') {
        meta.id = undefined;
        meta.path = locationPath;
      } else {
        out.appendLine(
          `skipped ${filePath}: front matter/location path mismatch not resolved`
        );
        return;
      }
    }
  ...
  if (meta.id) {
    if (meta.updatedAt && !opts.skipConflictPrompt) {
      const remoteUpdatedAt = await getPageUpdatedAt(ctx.url, token, meta.id);
      if (remoteUpdatedAt && !sameInstant(remoteUpdatedAt, meta.updatedAt)) {
        const choice = await vscode.window.showWarningMessage(
          `Wiki.js Sync: "${meta.path}" was changed on the server since you last synced (server copy updated ${remoteUpdatedAt}). Overwrite the server copy with your local file?`,
          { modal: true },
          'Overwrite'
        );
        if (choice !== 'Overwrite') {
